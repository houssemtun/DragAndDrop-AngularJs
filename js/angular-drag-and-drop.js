  ...
  return {
    ...
    link: function(scope, element, attrs, ngDragAndDrop) {
      ...
      scope.itemDropped = function(item) {
        var itemId = item.dragId;
        var newPos = getDroppedPosition(item);
        var items = document.getElementsByTagName("drag-item");
        for (var i = 0; i < items.length; i++) {
          if (items[i].attributes['drag-id'].value == itemId) {
            var dropCircle = document.getElementsByClassName("drop-content")[0];
            console.log(dropCircle.getBoundingClientRect().top);
            var p = items[i].querySelector('p');
            var image = items[i].querySelector('img');
            var imgUrl = image.attributes['src'].value;
            var title = p.innerHTML;
            var newItem = document.createElement("div");
            newItem.style.position = "absolute";
            newItem.style.marginTop = (item.top - dropCircle.getBoundingClientRect().top) + "px";
            newItem.style.marginLeft = (item.left - dropCircle.getBoundingClientRect().left) + "px";
            newItem.innerHTML = "<p style='display:none;'>" + title + "</p><img src=" + imgUrl + ">";
            dropCircle.appendChild(newItem);
          }
        }
